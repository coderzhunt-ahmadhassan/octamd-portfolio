      ...
      <div className='mt-[120px] mb-[50px] rounded-[30px] shadow-cardShadow'>
        <div className='grid grid-cols-1 lg:grid-cols-2'>
          <div className=' py-5 md:py-[60px] pl-5 md:pl-[60px] pr-5 md:pr-[60px] lg:pr-0'>
            <h2 className='urbanist font_48_700 text-center lg:text-left mt-[15px] mb-[35px] text-titleColor'>
              Get In Touch!
            </h2>
            <form className='grid grid-cols-1  gap-5'>
              ...
              />
              <Button
                className='rounded-[30px] col-span-1 dmSans font_14_700 h-14 border border-themePrimary w-full hover:bg-white hover:text-themePrimary'
                type='submit'
                variant={'primary'}
              >
                BOOK AN APPOINTMENT
  ...
  return (
    <div className='flex flex-col gap-[10px]'>
      <div className='shadow-cardShadow px-10 rounded-[30px] relative'>
        ...
        ></div>
        <h4 className='p-6 text-center urbanist font_24_700 border-b border-[#D8DDE1]'>{name}</h4>
        <div className='my-8 flex flex-col gap-8'>
          <div className='flex gap-1.5 justify-start items-start'>
            <GrLocation className='text-themePrimary relative top-1 w-5' />
            <p className='flex-1 urbanist font_16_400 text-[#788094]'>{address}</p>
          </div>
          <div className='flex gap-1.5 justify-start items-start'>
            <FaRegEnvelope className='text-themePrimary relative top-1 w-5' />
            <p className='flex-1 urbanist font_16_400 text-[#788094]'>{email}</p>
          </div>
          <div className='flex gap-1.5 justify-start items-start'>
            <TbPhoneCall className='text-themePrimary relative top-1 w-5' />
            <p className='flex-1 urbanist font_16_400 text-[#788094]'>{number}</p>
          </div>
        </div>
      </div>
      <div className='shadow-cardShadow p-10 rounded-[30px] flex flex-wrap gap-5'>
        ...
        <div className='flex flex-col justify-between items-start'>
          <h4 className='text-left urbanist font_20_700'>Opening Hours</h4>
          {openings?.map(opening => {
            if (opening.length > 1) {
              return (
                <p key={opening[0]} className='urbanist font_16_400 text-[#788094]'>
                  <span>{opening[0]}</span>: <span>{opening[1]}</span>
                </p>
              )
            } else if (opening.length === 1) {
              return (
                <p key={opening[0]} className='urbanist font_16_400 text-themePrimary'>
                  <span>{opening[0]}</span>
                </p>
              )
            } else return null
